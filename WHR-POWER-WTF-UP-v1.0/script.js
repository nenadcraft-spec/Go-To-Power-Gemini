...
/* =========================================================
   WHR: POWER WTF UP v1.4.0
   HYBRID PANG ENGINE WITH POWER-UPS & SLIDER JOYSTICK
========================================================= */
...
function updateGame(delta, timestamp) {
    ...
    // Power-ups Falling Update
    for (let pIdx = state.powerUps.length - 1; pIdx >= 0; pIdx--) {
        const p = state.powerUps[pIdx];
        p.y += 180 * delta;
        ...
        if (
            ...
            applyPowerUp(p.type);
            state.powerUps.splice(pIdx, 1);
        } else if (p.y > state.height) {
            state.powerUps.splice(pIdx, 1);
        }
    }

    // Collision Cables vs Orbs
...
function destroyOrb(idx, orb) {
    ...
    // Chance to drop WTF Power-up Item
    if (Math.random() < 0.25) {
        state.powerUps.push({
            x: orb.x,
            y: orb.y,
...
function renderGame() {
    ...
    state.powerUps.forEach(p => {
        ...
        ctx.fillStyle = p.type === "shield" ? "#32ff9b" : "#ffe45c";
        ctx.shadowColor = p.type === "shield" ? "#32ff9b" : "#ffe45c";
        ctx.shadowBlur = 12;
        ctx.fill();
        ctx.fillStyle = "#000";
        ctx.font = "bold 10px sans-serif";
        ctx.textAlign = "center";
        ctx.fillText(p.type === "shield" ? "S" : "*", p.x, p.y + 3);
    });
    ...
    if (state.player) {
        ...
        if (state.shieldActive) {
            ctx.beginPath();
            ctx.arc(state.player.x + state.player.width / 2, state.player.y + state.player.height / 2, 32, 0, Math.PI * 2);
