...
gameState.hideTimeout = setTimeout(
    function () {
        ...
        const portalStillMatches =
            randomPortal.classList.contains("active") &&
            randomPortal.dataset.entity ===
                expectedEntityKey;

        if (!portalStillMatches) {
            return;
        }
...
function removeEntity(portal) {
    if (!portal) {
        return;
    }

    const entityElement =
        portal.querySelector(".entity");
    ...
    delete portal.dataset.entity;

    if (gameState.currentPortal === portal) {
        gameState.currentPortal = null;
        gameState.currentEntity = null;
    }
}
...
function removeCurrentEntity() {
    removeEntity(gameState.currentPortal);
}
/* -------------------------
...
function playSpawnSound(entityType) {
if (!audioContext) return;
playSound(entityType.points > 0 ? 440 : 220, 0.05, "sine", 0.03);
}
/* -------------------------
HUD
------------------------- */
